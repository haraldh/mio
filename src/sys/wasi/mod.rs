...
fn combine_events(events: &mut Events, wasi_events: &[wasi::Event]) -> io::Result<()> {
    for wasi_event in wasi_events {
        ...
        match events.iter_mut().find(predicate) {
            None => events.push(wasi_event.clone().into()),
            Some(event) => {
                event.combine_other(wasi_event).unwrap();
            }
        }
    }
        ...
        match wasi_event.type_ {
            wasi::EVENTTYPE_FD_READ => match self {
                CombinedEvent {
                    ...
                    self.read.replace(wasi_event.clone());
                    Some(self)
                }
                _ => None,
            },
            wasi::EVENTTYPE_FD_WRITE => match self {
                CombinedEvent {
                    ...
                    self.write.replace(wasi_event.clone());
                    Some(self)
                }
                _ => None,
            },
